...
- **關卡不靠 AI 感覺** — 每張圖、每個 OCR 頁面都先過阻擋型 QC，而且 QC 鏈全是純計算、沒有模型參與。閾值**永遠不為了讓失敗案例過關而調整**
...
每本書的圖片排版邏輯都不同，沒有單一裁切規則。我們用**通用的幾何比對方法**（圖說認領最近的可指派圖像），後面接一道**決定論 QC 閘門**：兩項檢查——留白填充與文字滲入——都是純計算，所以同一張圖永遠得到同一個判定。QC 全程沒有模型參與：問模型的檢查曾讓同一張圖這次過、下次不過（issue #16），所以模型檢查被整段移出 QC 鏈，抽圖路徑完全不需要本地視覺模型。閘門**寧可 hard-fail 也不亂猜**，所以指錯頁碼會得到拒絕而不是錯圖。至於一張過了 QC 的圖「值不值得嵌入」是另一個問題，由 frontier 級模型在嵌入前逐張閱讀分類（見 workflow 的分類步驟）。
...
- 選用（處理掃描書的 OCR 階梯與語意搜尋）：NVIDIA GPU 或 Apple Silicon + [Surya OCR](https://github.com/VikParuchuri/surya)、[ollama](https://ollama.com) 加小視覺模型與 `bge-m3` embeddings，全部本機執行，資料不出你的電腦。抽圖與其 QC 閘門完全不需要這些（0.7.1 起為純計算）。見 [`docs/ocr-ladder.md`](docs/ocr-ladder.md) 的硬體分級表
