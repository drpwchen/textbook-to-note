...
| `text_contamination` (OCR long-lines) | Legacy check — a vision-model OCR line count | **Removed from the QC chain in 0.7.1** (issue #16: the count is not reproducible between runs, so it can neither gate nor serve as a log signal). The function remains as an opt-in diagnostic only. The blocking guard against a crop that swallowed body text is `text_bleed`, which reads the PDF's own text layer. |
